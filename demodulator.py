...
class Demodulator:
    ...
    def __generate_complex_16psk(self, data_signal):
        ...
        for bit in range(0, len(sinwave), self.__period * self.__sample_rate):
            ...
            maxval = max(coeff1, coeff2, coeff3, coeff4, coeff5, coeff6, coeff7, coeff8, coeff9, coeff10, coeff11, coeff12, coeff13, coeff14, coeff15, coeff16)
    ...
    def make_8psk_demod(self, data_signal: WirelessSignal, channel: Channel):
        ...
        for com in range(len(phi_of_complex_numbers)):
            ...
            elif phi_of_complex_numbers[com] >= np.pi - 2 * np.pi and phi_of_complex_numbers[com] < 5 * np.pi / 4 - 2 * np.pi :
                result_data_bits.extend([1, 0, 0])
            elif phi_of_complex_numbers[com] >= 5 * np.pi / 4 - 2 * np.pi and phi_of_complex_numbers[com] < 3 * np.pi / 2 - 2 * np.pi :
                result_data_bits.extend([1, 0, 1])
            elif phi_of_complex_numbers[com] >= 3 * np.pi / 2 - 2 * np.pi and phi_of_complex_numbers[com] < 7 * np.pi / 4 - 2 * np.pi :
                result_data_bits.extend([1, 1, 0])
            elif phi_of_complex_numbers[com] >= 7 * np.pi / 4 - 2 * np.pi and phi_of_complex_numbers[com] < 2 *np.pi - 2 * np.pi :
    ...
    def make_16psk_demod(self, data_signal: WirelessSignal, channel: Channel):
        ...
        for com in range(len(phi_of_complex_numbers)):
            ...
            elif phi_of_complex_numbers[com] >= np.pi - 2 * np.pi  and phi_of_complex_numbers[com] < 9 * np.pi / 8  - 2 * np.pi:
                result_data_bits.extend([1, 0, 0, 0])
            elif phi_of_complex_numbers[com] >= 9 * np.pi / 8 - 2 * np.pi and phi_of_complex_numbers[com] < 5 * np.pi / 4 - 2 * np.pi :
                result_data_bits.extend([1, 0, 0, 1])
            elif phi_of_complex_numbers[com] >= 5 * np.pi / 4 - 2 * np.pi and phi_of_complex_numbers[com] < 11 * np.pi / 8 - 2 * np.pi :
                result_data_bits.extend([1, 0, 1, 0])
            elif phi_of_complex_numbers[com] >= 11 * np.pi / 8 - 2 * np.pi and phi_of_complex_numbers[com] < 3 * np.pi / 2 - 2 * np.pi :
                result_data_bits.extend([1, 0, 1, 1])
            elif phi_of_complex_numbers[com] >= 3 * np.pi / 2 - 2 * np.pi and phi_of_complex_numbers[com] < 13 * np.pi / 8 - 2 * np.pi :
                result_data_bits.extend([1, 1, 0, 0])
            elif phi_of_complex_numbers[com] >= 13 * np.pi / 8 - 2 * np.pi and phi_of_complex_numbers[com] < 7 * np.pi / 4 - 2 * np.pi :
                result_data_bits.extend([1, 1, 0, 1])
            elif phi_of_complex_numbers[com] >= 7 * np.pi / 4 - 2 * np.pi and phi_of_complex_numbers[com] < 15 * np.pi / 8 - 2 * np.pi :
                result_data_bits.extend([1, 1, 1, 0])
            elif phi_of_complex_numbers[com] >= 15 * np.pi / 8 - 2 * np.pi and phi_of_complex_numbers[com] < 2 *np.pi - 2 * np.pi :
